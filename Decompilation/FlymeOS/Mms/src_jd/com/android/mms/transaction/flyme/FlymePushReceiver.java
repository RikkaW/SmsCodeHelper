...
public class FlymePushReceiver
  extends BroadcastReceiver
{
  static final String[] a = { "address", "delivery_status", "date" };
  static final String[] b = { "address", "d_rpt", "date" };
  
  private boolean a(String paramString, long paramLong)
  {
    return (paramString.contains("talk#audio")) || (((paramString.contains("img#image")) || (paramString.contains("location#image"))) && (paramLong < 2097152L));
  }
  
  private static b b(Context paramContext, MzGenericPdu paramMzGenericPdu, int paramInt)
  {
    b localb = new b(null);
    if (localb != null)
    {
    ...
    label190:
    for (paramMzGenericPdu = new String(((MzDeliveryInd)paramMzGenericPdu).getMessageId());; paramMzGenericPdu = new String(((MzReadOrigInd)paramMzGenericPdu).getMessageId()))
    {
      paramContext = paramContext.getContentResolver();
      Uri localUri = Telephony.Mms.CONTENT_URI;
      paramMzGenericPdu = "m_id = " + DatabaseUtils.sqlEscapeString(paramMzGenericPdu) + " AND " + "m_type" + " = " + 128;
      paramContext = paramContext.query(localUri, new String[] { "thread_id", "_id" }, paramMzGenericPdu, null, null);
      if (paramContext != null) {}
      try
      {
        if ((paramContext.getCount() == 1) && (paramContext.moveToFirst()))
        {
          a = paramContext.getLong(0);
          b = paramContext.getLong(1);
        }
        return localb;
      }
      finally
      ...
      Log.e("FlymePushReceiver", "new delive failed");
      return localb;
    }
  }
  
  private static boolean b(Context paramContext, MzNotificationInd paramMzNotificationInd)
  {
    paramMzNotificationInd = paramMzNotificationInd.getContentLocation();
    if (paramMzNotificationInd != null)
    {
      paramMzNotificationInd = new String(paramMzNotificationInd);
      paramContext = paramContext.getContentResolver().query(Telephony.Mms.CONTENT_URI, new String[] { "_id" }, "ct_l = ?", new String[] { paramMzNotificationInd }, null);
      if (paramContext == null) {}
    }
    try
    {
  ...
  {
    Log.e("FlymePushReceiver", "Received PUSH Intent: " + paramIntent);
    if ((paramIntent.getAction().equals("android.provider.Telephony.WAP_PUSH_RECEIVED")) && ("application/vnd.flyme.mms-message".equals(paramIntent.getType())))
    {
      ((PowerManager)paramContext.getSystemService("power")).newWakeLock(1, "MMS FlymePushReceiver").acquire(5000L);
      new a(paramContext).execute(new Intent[] { paramIntent });
    }
  }
    ...
    protected Void a(Intent... paramVarArgs)
    {
      ...
      if (!FlymePushReceiver.a(b, (MzNotificationInd)localObject5))
      {
        ...
        if (str.contains("#video"))
        {
          ...
          if (!FlymePushReceiver.a(FlymePushReceiver.this, str, l1)) {
            break label1498;
          }
          ...
          if ((FlymePushReceiver.a(FlymePushReceiver.this, str, l1)) || ((bool3) && (bool2) && (bool1))) {
            break label1604;
          }
...
/* Location:
 * Qualified Name:     com.android.mms.transaction.flyme.FlymePushReceiver
